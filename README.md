...
| :---- | :----- | :---- |
| io.moyada.medivh.annotation.NotNull | field | indicates whether or not this field not allowed to be null, default when use any Rule. |
| io.moyada.medivh.annotation.Nullable | field | indicates whether or not this field allowed to be null, except primitive type, prior to NotNull. |
| io.moyada.medivh.annotation.NumberRule | field | provide validate rule for enclose class. |
| io.moyada.medivh.annotation.SizeRule | field | provide validate rule for enclose class. |
...
| Check.throwable() | configure the action when the argument is invalid, true is to throw exception, false is to return value, default to true. |
| Check.nullable() | indicates whether or not this parameter allowed to be null, default to false. |
| Check.invalid() | configure thrown exception, the exception type must have a String constructor, default is IllegalArgumentException. |
| Check.returnValue() | set the return value, support return type as primitive type or object, the returnValue needs to have a correspond constructor when return type is object. |
...
public class MyApp {
    ...
    public Info run(@Check(invalid = RuntimeException.class) Args args,
                    ...
                    @Check(throwable = false, returnValue = {"test", "-0.503"}) String name, // can be check null value for normal Object

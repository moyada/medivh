...
| 注解 | 作用域 | 作用 |
| :---- | :----- | :---- |
| @Nullable | 类字段、无参方法、方法参数 | 设置对象类型不进行非空校验。 |
| @NotNull | 类字段、无参方法 | 设置对象类型进行非空校验，默认定义了规则都会进行非空校验。 |
| @NotBlank | 类字段、无参方法、方法参数 | 定义 String 类型不可为空白字符串。 |
| @SizeRule | 类字段、无参方法、方法参数 | 为 String 或 数组 或 集合 类型提供校验规则。 |
| @NumberRule | 类字段、无参方法、方法参数 | 为基础数字类型提供校验规则。 |
| @Throw | 类、非静态方法、方法参数 | 配置参数的校验处理逻辑，当校验失败时将抛出异常，可作用于类或方法上设置默认处理逻辑。 |
| @Return | 非静态方法、方法参数 | 对拥有返回值的方法配置参数的校验处理逻辑，当校验失败时将返回指定数据，可作用于方法上设置默认处理逻辑。 |
| @Exclusive | 方法、方法参数 | 排除校验，可用于禁用默认处理逻辑。 |
| @Variable | 非静态方法、类 | 修改当前作用域下校验逻辑产生的临时变量名和方法名。 |

* 使用示例见[这里](#示例)和[WIKI](https://github.com/moyada/medivh/wiki)。

属性说明

| 属性 | 作用 |
| :--- | :--- |
| NumberRule.min() | 设置数字类型允许的最小值。 |
| NumberRule.max() | 设置数字类型允许的最大值。 |
| SizeRule.min() | 设置 String、数组、集合 类型允许的最小长度或容量。 |
| SizeRule.max() | 设置 String、数组、集合 类型允许的最大长度或容量。 |
| Throw.value() | 设置抛出异常类，异常类需要拥有字符串构造方法。 |
| Throw.message() | 指定异常信息头。 |
| Return.value() | 设置返回值，当返回类型为对象时需要有对应构造函数。 |
| Return.type() | 指定返回值的类型，需为方法返回类型的子类或实现类。 |
| Variable.value() | 指定方法生成逻辑时产生的元素名称。 |
...
[更多用法](https://github.com/moyada/medivh/wiki)
